...
연산자를 입력했을때 PushOperand -> TryCalc -> push -> deletebuffer의 순서대로 일어난다.(ETPD)
...
ㄴ Calculate를 1회 수행 한다.						(case2)
elseif 스택에 연산자가 1개있고 그 연산자가 +, - 인경우
ㄴ Calculate를 1회 수행 한다.						(case3)
ㄴ push 할 연산자가 *, / 인경우 return					(case4)
elseif 스택에 연산자가 2개있고, push하려는 연산자가 *, / 인경우				
ㄴ Calculate를 1회 수행 한다.						(case5)
elseif 스택에 연산자가 2개있고, push하려는 연산자가 +, - 인경우				
ㄴ 스택이 빌 때까지 Calculate한다.						(case6)

[Calculate]
- (피연산자 pop) (연산자 pop) (피연산자 pop) 의 연산을 하여 결과값을 피연산자 스택에 push 한다.
- 연산결과를 Screen에 출력하고 enqueue(연산결과)를 호출한다.
